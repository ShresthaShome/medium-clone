<ul
    ...
    <li class="me-2">
        <a href="{{ route('dashboard') }}" aria-current="page"
            class="{{ request('categoryName') ? 'inline-block p-2 rounded-t-lg hover:text-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800 dark:hover:text-gray-300' : 'inline-block p-2 text-blue-600 bg-gray-100 rounded-t-lg active dark:bg-gray-800 dark:text-blue-500 border-b-2 border-blue-600' }}">All</a>
    </li>
    @forelse ($categories as $category)
        <li class="me-2">
            <a href="{{ route('post.byCategory', strtolower($category->name)) }}"
                class="{{ Route::currentRouteNamed('post.byCategory') &&
                strtolower(request('categoryName')) == strtolower($category->name)
